<?php
	...
	class Trainer_model extends CI_Model {
		...
		//	print_r($this->db->query("SELECT id_vk FROM users WHERE id_vk='$id_vk'")->result()[0]->id_vk->count());
		  $result=$this->db->query("SELECT teacher.id,teacher.name,teacher.sername,teacher.otch,section.name,club.address AS section_name,club.name AS club_name,teacher.rating FROM teacher,section,club WHERE teacher.id_section=section.id AND section.id_club=club.id ORDER BY teacher.rating DESC")->result();
      return $result;
	}
